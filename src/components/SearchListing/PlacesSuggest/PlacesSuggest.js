...
class PlacesSuggest extends React.Component {
  ...
  async handleSelectSuggest(data) {
    ...
    let locationData = {};
    if (data && data.gmaps) {
      let viewport, sw_lat, sw_lng, ne_lat, ne_lng;
      viewport = data.gmaps.geometry.viewport;
      let northEast = data.gmaps.geometry.viewport.getNorthEast();
      let southWest = data.gmaps.geometry.viewport.getSouthWest();
