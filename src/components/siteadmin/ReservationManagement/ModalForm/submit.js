...
async function submit(values, dispatch) {
  let paymentCurrency =
    values.paymentMethodId == 1 ? values.paymentCurrency : null

  if (values.type === "host") {
    paymentCurrency =
      values.paymentMethodId == 3 ? values.payoutCurrency : paymentCurrency
    if (values.changeState)
      values.changeState("addPayout", values.reservationId)
    dispatch(
      payoutHost(
  ...
  if (values.type === "guest") {
    if (values.changeState)
      values.changeState("addRefund", values.reservationId)
    dispatch(
      refundGuest(
        values.reservationId,
        values.receiverEmail,
