...
import { User, UserProfile, UserVerifiedInfo, EmailToken, SiteSettings } from '../data/models';
...
const getData = async () => {
  const results = await SiteSettings.findAll({
    ...
    where: {
      name: {
        $in: ['facebookAppId', 'facebookSecretId']
      }
    }
  });
...
getData().then(function () {
  passport.use(new FacebookStrategy({
    clientID: settingsData.facebookAppId,
    clientSecret: settingsData.facebookSecretId,
