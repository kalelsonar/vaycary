    ...
    try {
      ...
      if (requestHeader && requestData &&
        ...
        if (requestData.to && requestData.type) {
          ...
          status = 400;
          errorMessage = await showErrorMessage({ errorCode: 'templateMissing' })
        }
      } else {
        status = 400;
          ...
          if (isListingAvailable) {
            ...
            if (getReservationCount > 0) {
              ...
              if (listPhotosCount <= 1) {
                ...
                status = 400;
                errorMessage = await showErrorMessage({ errorCode: 'removeListPhoto' })
            ...
            if (checkForRemove) {
              ...
              if (removePhoto) {
                ...
                status = 400;
                errorMessage = await showErrorMessage({ errorCode: 'existFileName' })
              }
              res.send({ status, errorMessage });
            }
          } else {
            status = 400;
            errorMessage = await showErrorMessage({ errorCode: 'isListingAvailable' })
            res.send({ status, errorMessage });
          }
        } else {
          status = 400;
          errorMessage = await showErrorMessage({ errorCode: 'userExist' })
          res.send({ status, errorMessage });
        }
      }
    } catch (error) {
      ...
      if (requestHeader &&
        ...
        if (userLogin) {
          ...
          if (parsedFiles != undefined && parsedFiles.length > 0) {
            ...
            status = 200;
            errorMessage = await showErrorMessage({ errorCode: 'removeFiles' })
          } else {
            status = 400;
            errorMessage = await showErrorMessage({ errorCode: 'removeFilesError' })
          }
        } else {
          status = 400;
          errorMessage = await showErrorMessage({ errorCode: 'userExist' })
