...
const blockImportedDates = {
    ...
    args: {
        ...
        blockedDates: { type: new List(
            new InputObjectType({
                name: 'blockedDatesType',
                fields: {
            ...
            if (isListingAvailable) {
                ...
                if (blockedDates && blockedDates.length > 0) {
                    ...
                    blockedDates.map(async (item) => {
                        let oldRecords = reservationDatesCollection.find((v) => moment(item?.date).format('YYYY-MM-DD') === moment(v.blockedDates).format('YYYY-MM-DD'));
                        let checkDate = reservationDatesCollection.indexOf(item?.date);
                        ...
                        if (checkDate === -1 && !oldRecords) {
                            blockedDatesInstance = {
                                ...
                                listId,
                                calendarId,
                                blockedDates: moment(item?.date).format('YYYY-MM-DD'),
