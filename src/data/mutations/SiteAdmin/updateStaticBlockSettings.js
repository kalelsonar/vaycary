...
const updateStaticBlockSettings = {
    ...
    async resolve({ request }, {
        ...
        if (request.user && request.user.admin == true) {
            ...
            const updateBlock1 = await StaticInfoBlock.update({ title: blockTitle1, content: blockContent1, image: blockImage1, isEnable: isEnable }, { where: { name: 'block1' } })
                .then(function (instance) {
                    // Check if any rows are affected
                    if (instance > 0) {
                        isStaticBlockSettingsUpdated = true;
            ...
            const updateBlock2 = await StaticInfoBlock.update({ title: blockTitle2, content: blockContent2, image: blockImage2, isEnable: isEnable }, { where: { name: 'block2' } })
                .then(function (instance) {
                    // Check if any rows are affected
                    if (instance > 0) {
                        isStaticBlockSettingsUpdated = true;
