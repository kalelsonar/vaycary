...
const userEditProfile = {
  ...
  async resolve({ request, response }, {
    ...
    if (request.user && request.user.admin != true) {
      ...
      const getPhoneNumbStatus = await SiteSettings.findOne({
        attributes: ['value'],
        where: {
      ...
      let isPhoneNumber = getPhoneNumbStatus?.value === '2' ? phoneNumber : isUserPhoneNumber.phoneNumber;
      let isCountryCode = getPhoneNumbStatus?.value === '2' ? countryCode : isUserPhoneNumber.countryCode;
      let isCountryName = getPhoneNumbStatus?.value === '2' ? countryName : isUserPhoneNumber.countryName;
