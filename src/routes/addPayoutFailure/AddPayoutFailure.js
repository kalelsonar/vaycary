...
class AddPayoutFailure extends React.Component {
  ...
  render() {
    const { siteName, currentAccountId, userId } = this.props
        ...
        <Grid fluid>
          <Row className={cx(s.space8, s.spaceTop8)}>
            <Col xs={12} sm={12} md={12} lg={12} className={s.textCenter}>
              ...
              <h2>
                <FormattedMessage {...messages.payoutFailureSubtitle} />
              </h2>
              <p className={s.marginTop20}>
                <FormattedMessage {...messages.payoutFailureContent} />
              </p>
...
export default injectIntl(
  withStyles(s, bt)(connect(mapState, mapDispatch)(AddPayoutFailure))
)
